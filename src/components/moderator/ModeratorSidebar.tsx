...
const modNavItems = [
  ...
  { icon: Flag, label: 'Fila de Moderação', href: '/moderator/queue', badge: 12 },
      ...
      <div className="flex h-full flex-col">
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto p-4">
          {modNavItems.map((item) => {
            const isActive = location.pathname === item.href || 
              (item.href !== '/moderator' && location.pathname.startsWith(item.href));
        ...
        <div className="mx-4 mb-4 rounded-lg border border-border bg-muted/50 p-3">
          <p className="text-xs text-muted-foreground">
            <strong className="text-foreground">Dica:</strong> Priorize denúncias com mais de 24h sem resposta.
          </p>
        </div>
